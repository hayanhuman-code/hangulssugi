...
- **탭이 다섯에서 일곱으로 늘었다.** 세 화면 분기의 글자·아이콘을 줄였고, 폰(560px 이하)
  에서는 두 줄(4+3)로 접는다. 카드 줄 수는 `rowsFor()` 가 남은 높이에서 다시 잡으므로
  접힌 만큼 그리드가 알아서 줄어든다.
  - **561~1023px 구간에 탭 분기가 없었다.** 가로로 누운 폰(844×390·667×375)과 작은 창이
    여기 걸린다. 다섯일 때는 기본 크기로도 남았지만 일곱이 되자 이름표가 서로 밀려
    겹쳤다 — 844 에서 탭 하나에 100px 뿐인데 내용은 그보다 넓었다. 이 구간을 62px 높이 ·
    18px 글자로 줄였다.
  - 여기서는 접을 수 없다. 가로로 누운 폰은 높이가 390px 뿐이라 탭 한 줄을 더 얹으면
