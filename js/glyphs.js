...
(function (global) {
  ...
  /* 겹모음은 가로모음 + 세로모음으로 이루어진다.
     음절에 들어갈 때 두 부분이 서로 다른 자리를 차지하므로 조합을 기억해 둔다. */
  var PARTS = {
    'ㅘ': ['ㅗ', 'ㅏ'], 'ㅙ': ['ㅗ', 'ㅐ'], 'ㅚ': ['ㅗ', 'ㅣ'],
    'ㅝ': ['ㅜ', 'ㅓ'], 'ㅞ': ['ㅜ', 'ㅔ'], 'ㅟ': ['ㅜ', 'ㅣ'],
    'ㅢ': ['ㅡ', 'ㅣ']
  ...
  /* 낱자로 보여 줄 때의 겹모음 — 가로모음은 왼쪽 아래, 세로모음은 오른쪽 전체 */
  Object.keys(PARTS).forEach(function (key) {
    var h = VOW[PARTS[key][0]], v = VOW[PARTS[key][1]];
    VOW[key] = cat(box(h, 40, 400, 560, 920), box(v, 540, 60, 980, 940));
  });
  ...
  var DIGIT = {
    ...
    '8': ['M650,260 C590,185 430,180 370,260 C305,330 360,420 500,500 ' +
          'C600,555 725,610 730,710 C735,825 630,890 500,890 ' +
          'C370,890 265,825 270,710 C275,610 400,555 500,500 ' +
          'C600,445 670,340 650,260'],
  ...
  /*
   * 초성 → 중성 → 종성 순서로 획을 이어 붙인다.
   * 자리 배치는 한글 조합 규칙 그대로 —
   *   세로모음(가) : 초성 왼쪽,  모음 세로획은 글자 전체 높이
   *   가로모음(고) : 초성 위,    모음은 아래 전체 폭
  ...
  function syllable(ch) {
    var p = decompose(ch);
    if (!p) return null;
    var c = CONS[p.cho], f = p.jong ? CONS[p.jong] : null;
    if (!c) return null;

    var parts = PARTS[p.jung];
    var jung = p.jung;
    ...
    var roomy = TALL_WIDE.indexOf(jung) >= 0;
    if (!f) {
      return roomy
        ? cat(box(c, 60, 90, 470, 850), box(vow, 450, 50, 960, 950))
        : cat(box(c, 70, 90, 540, 850), box(vow, 510, 50, 950, 950));
    }
    return roomy
      ? cat(box(c, 60, 60, 430, 530), box(vow, 410, 30, 950, 565), box(f, 195, 570, 805, 955))
      : cat(box(c, 70, 60, 480, 530), box(vow, 460, 30, 930, 565), box(f, 195, 570, 805, 955));
  }
